...
export default function ChatPanel({ messages, loading, onSend }: Props) {
  ...
  const handleSend = (e?: React.FormEvent) => {
    ...
    const query = mode === "lookalike" ? `lookalike:${input.trim()}` : input.trim();
    onSend(query);
    setInput("");
...
function EmptyState({ onSend }: { onSend: (q: string) => void }) {
  return (
    <div style={{ padding: "28px 16px" }}>
      ...
      <div style={{ ...S, fontSize: 16, fontWeight: 600, color: "var(--fg)", marginBottom: 6, lineHeight: 1.3 }}>
        Copilot M&A
      </div>
      ...
      {EXAMPLES.map((ex, idx) => (
        ...
        >
          <span style={{ ...M, fontSize: 9, color: "var(--fg-dim)", flexShrink: 0 }}>
            {String(idx + 1).padStart(2, "0")}
          </span>
          <span style={{ ...S, fontSize: 12, color: "var(--fg-muted)", flex: 1, lineHeight: 1.5 }}>
            {ex}
          </span>
        </button>
      ))}
      ...
      {(msg.actions && msg.actions.length > 0 || isStreaming) && (
        <div style={{ marginBottom: 12 }}>
          ...
          >
            ...
            {!isStreaming && (
              <ChevronDown size={11} style={{ transform: actionsOpen ? "none" : "rotate(-90deg)", transition: "transform 0.15s" }} />
            )}
          </button>
        ...
        <div style={{ ...S, fontSize: 13, color: "var(--fg)", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
          ...
          {isStreaming && (
            <span style={{
              ...
              background: "var(--fg)", marginLeft: 2,
